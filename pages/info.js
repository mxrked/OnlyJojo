// Next/React stuff
import Head from "next/head";

// Image Imports
import { FAVICON, APPLE_TOUCH_ICON, F16, F32 } from "../assets/cdns/images/tab";

// Asset Imports
import CheckingForCartList from "../assets/functions/CheckingForCartList";
import { CartList } from "../assets/data/arrays/CartList";
import InfoJP from "../components/translations/InfoJP";

// Component Imports
import OnAllPages from "../components/all/OnAllPages";
import Navigation from "../components/layout/nav/Navigation";

import Top from "../components/views/info/Top";
import Background from "../components/views/info/Background";
import Jojos from "../components/views/info/Jojos";

import Footer from "../components/layout/footer/Footer";

// Styles
import styles from "../styles/views/info/Info.module.css";

export default function Info() {
  CheckingForCartList();

  console.table(CartList);

  return (
    <div id={styles.PAGE_CNT}>
      <Head>
        <title>OnlyJojo | A Bizarre Online Store - About Us</title>

        {/* SEO 
        <meta name="description" content="" />
        <meta name="keywords" content="" />
        <meta name="robots" content="index, follow" />
        <meta
          property="og:title"
          content="OnlyJojo | A Bizarre Online Store - About Us"
        />
        <meta property="og:type" content="website" />
        <meta property="og:locale" content="en_GB" />
        <meta
          property="og:url"
          content="http://basicallyeasy.com/next-projects/onlyjojo/info/"
        />
        */}
        <meta name="robots" content="index, follow" />

        <link
          rel="canonical"
          href="http://basicallyeasy.com/next-projects/onlyjojo/info/"
        />

        {/* Tab Icons */}
        <link rel="shortcut icon" href={FAVICON} />
        <link rel="apple-touch-icon" sizes="180x180" href={APPLE_TOUCH_ICON} />
        <link rel="icon" type="image/png" sizes="32x32" href={F32} />
        <link rel="icon" type="image/png" sizes="16x16" href={F16} />

        {/* Google Fonts */}
        <link rel="preconnect" href="https://fonts.googleapis.com" />
        <link rel="preconnect" href="https://fonts.gstatic.com" crossOrigin />
        <link
          href="https://fonts.googleapis.com/css2?family=Poppins:wght@400;700;900&display=swap"
          rel="stylesheet"
        ></link>
      </Head>

      {/* Global Components/Functions */}
      <OnAllPages />
      <Navigation />

      <InfoJP />

      <div id={styles.infoCnt} className="overrides_Info">
        <Top />
        <Background />
        <Jojos />
      </div>

      {/* Footer */}
      <Footer />
    </div>
  );
}
